...
namespace FaceLivenessWPF
{
    internal class SeetaFace6SharpFactory : IDisposable
    {
        ...
        FaceAntiSpoofing _faceAntiSpoofing = null;

        AgePredictor _agePredictor = null;
        GenderPredictor _genderPredictor = null;
        EyeStateDetector _eyeStateDetector = null;
        //人脸追踪
        ...
        public T Get<T>() where T : class
        {
            switch (typeof(T).Name)
            {
                ...
                case nameof(FaceAntiSpoofing):
                    {
                        if (_faceAntiSpoofing == null)
                            _faceAntiSpoofing = new FaceAntiSpoofing();
                        return _faceAntiSpoofing as T;
                    }
                case nameof(EyeStateDetector):
                    {
                        if (_eyeStateDetector == null)
                            _eyeStateDetector = new EyeStateDetector();
                        return _eyeStateDetector as T;
                    }
                default:
                    {
